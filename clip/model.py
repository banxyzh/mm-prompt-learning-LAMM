...
# class ResidualAttentionBlock_UnifiedNP(nn.Module):
#     def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
#         super().__init__()
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward_prev(self, x: torch.Tensor):
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return x
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp' or current_trainer == 'UnifiedNP'
...
class VisionTransformer_UnifiedNP(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward_tail(self, x: torch.Tensor):
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "UnifiedNP":
                self.visual = VisionTransformer_UnifiedNP(
                    input_resolution=image_resolution,
