...
class TripletLoss(nn.Module):
    ...
    def cal_dist(self, x1, x2):
        assert x1.shape == x2.shape, "Input dim is different"

        x1 = x1.view(1, -1)
        x2 = x2.view(1, -1)
        
        dist = torch.pow(x1 - x2, 2).sum(dim=1, keepdim=True)
        # dist = dist.clamp(min=1e-12).sqrt()
        dist = dist.clamp(min=1e-4).sqrt()
        
        return dist[0]
    
    def cal_dist_an(self, x1, x2):
        
        assert x1.shape[1] == x2.shape[1], "Input dim is different"

        dist = torch.pow(x1 - x2, 2).sum(dim=1, keepdim=True)
        dist = dist.clamp(min=1e-2).sqrt()
        
        # return dist.mean().reshape(1)
        return dist.min().reshape(1)
    
    def forward(self, text_features, fix_label_features, label):
        select_text_feature = text_features[label]

        loss_num = 0
        loss = torch.tensor(0.).to(text_features.device)
        for i in range(label.size(0)):
            if torch.sum(label !=  label[i]) > 0:
                dist_ap = self.cal_dist(text_features[i], fix_label_features[i])
                dist_an = self.cal_dist_an(text_features[i].unsqueeze(0), select_text_feature[label !=  label[i]])
                loss_num += 1
                print("dist_ap is ", dist_ap)
                print("dist_an is ", dist_an)
                
                # Compute ranking hinge loss
                for j in range(dist_an.size(0)):
                    dist_an_one = dist_an[j]
                    dist_an_one = dist_an
                    loss_num += 1
                    
                    # Compute ranking hinge loss
                    y = dist_an_one.data.new()
                    y.resize_as_(dist_an_one.data)
                    y.fill_(1)
                    y = Variable(y) # .to(dist_an.device)
                    loss += self.ranking_loss(dist_an_one, dist_ap, y)
        if loss_num != 0:
...
class PromptLearner(nn.Module):
    ...
    def _get_token_classes(self, dtype):
        ...
        if not self.eval_only:
            ...
            self.token_classes = self.token_classes_all[:, 1, :].unsqueeze(1)
            for i, one_name_len in enumerate(self.name_lens):
                self.token_classes[i, 0, :] = torch.mean(self.token_classes_all[i, 1:1+one_name_len, :], dim=0)
            self.token_classes = nn.Parameter(self.token_classes)
        else:
            pass
        
    def _get_token_classes_v2(self, dtype):
    ...
    def construct_prompts_v2(self, ctx, prefix, classes, suffix):
        ...
        # print("self.class_token_position is ", self.class_token_position) # end 
    ...
    def forward(self, image, label=None):
        ...
        if self.prompt_learner.training:
            if self.triplet_loss:
                ...
                triple_loss_fn = TripletLoss()
                triple_loss = triple_loss_fn(text_features, fix_label_features, label)
                
                entorpy_loss = F.cross_entropy(logits, label)
                print("entorpy_loss is {}, triple_loss is {}". format(entorpy_loss, triple_loss))
                
                return entorpy_loss + 0.1 * triple_loss
            else:
...
class CoOp(TrainerX):
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            if "token_classes" in state_dict:
                del state_dict["token_classes"]
